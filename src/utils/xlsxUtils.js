...
function numToString(num) {
  const strArray = []
  const numToStringAction = function(o) {
    const temp = o - 1
    const a = parseInt(temp / 26)
    const b = temp % 26
...
export function exportExcel(data, columns, filename = FILE_NAME, styleConf) {
  ...
  const colNames = _.mapValues(columns, o => {
  ...
  if (['xlsx', 'xlsm', 'xlsb'].includes(bookType)) {
    ...
    for (const key in ws) {
      ...
      if (key.replace(/[^0-9]/ig, '') === '1') {
        ...
        const str = key.replace(/[^A-Za-z]+$/ig, '')
        const colIndex = stringToNum(str) - 1
